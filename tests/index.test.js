...
test("single test", async () => {
	...
	// Extract timestamp from the XML for dynamic comparison
	const timestampMatch = xml.match(/timestamp="(\d+)"/);
	assert.ok(timestampMatch, "Should have timestamp attribute");
	const expectedTimestampValue = timestampMatch[1];
	...
	const expectedXml =
		...
		"\t<sources>\n" +
		"\t\t<source>/Users/bacebu4/dev/cobertura-test</source>\n" +
		"\t</sources>\n" +
		"\t<packages>\n" +
